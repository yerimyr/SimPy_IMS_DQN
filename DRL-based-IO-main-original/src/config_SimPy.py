...
DEMAND_SCENARIO = {
    "Dist_Type": "GAMMA",
    "Parameters": [
        {SIM_TIME: [1,30], "shape": 10, "scale": 1},  # 초기 구간
        {SIM_TIME: [31, 100], "shape": 30, "scale": 1}  # 후반 구간
    ]
}
...
LEADTIME_SCENARIO = {
    "Dist_Type": "GAMMA",
    "Parameters": [
        {SIM_TIME: [1, 50], "shape": 2, "scale": 1},
        {SIM_TIME: [51,100], "shape": 5, "scale": 1}
    ]
}
"""
...
def DEMAND_QTY_FUNC(scenario):
    ...
    # Gaussian distribution
        ...
        if demand < 0:
            ...
            return demand
    elif scenario["Dist_Type"] == "GAMMA":
        current_time = SIM_TIME  # 현재 시뮬레이션 시간
        params = next(param for param in scenario["Parameters"] if param[SIM_TIME][0] <= current_time <= param[SIM_TIME][1])
        demand = round(np.random.gamma(params["shape"], params["scale"]))
        if demand < 0:
            return 1
        elif demand > INVEN_LEVEL_MAX:
            return INVEN_LEVEL_MAX
...
def SUP_LEAD_TIME_FUNC(lead_time_dict):
    ...
    elif lead_time_dict["Dist_Type"] == "GAMMA":
        current_time = SIM_TIME  # 현재 시뮬레이션 시간
        params = next(param for param in lead_time_dict["Parameters"] if param[SIM_TIME][0] <= current_time <= param[SIM_TIME][1])
        lead_time = round(np.random.gamma(params["shape"], params["scale"]))
        if lead_time < 0:
            lead_time = 0
        elif lead_time > 7:
            lead_time = 7
